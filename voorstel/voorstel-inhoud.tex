...
Voetbalclubs verzamelen steeds grotere hoeveelheden data. Deze gegevens omvatten wedstrijdstatistieken, medische en fysieke meetwaarden, trainingsvolumes, GPS-tracking en wedstrijdbeelden.
Uit bestaande literatuur blijkt echter dat veel voetbalclubs deze data niet of slechts gedeeltelijk benutten, waardoor waardevolle inzichten verloren gaan ~\autocite{Fried2016}.
...
De centrale probleemstelling luidt dat voetbalclubs onvoldoende rendement halen uit de aanwezige data, ondanks het potentieel ervan.
Dit leidt tot suboptimale coachingbeslissingen, beperkte strategische inzichten en een lagere efficiëntie in de werking van een club.
De onderzoeksvraag die hieruit volgt is: \textbf{Hoe kunnen voetbalclubs hun bestaande data optimaler inzetten om de kwaliteit van besluitvorming en sportprestaties te verbeteren?}
...
Op basis van deze onderzoeksvraag worden verschillende deelvragen geformuleerd die het onderzoek wat meer richting geven.
In het probleemdomein wordt eerst nagegaan welke soorten data voetbalclubs vandaag verzamelen en op welke manier deze gegevens in de praktijk worden ingezet.
...
Vervolgens wordt aandacht besteed aan de voornaamste knelpunten die voetbalclubs ervaren bij het verwerken, analyseren en interpreteren van hun data.
Dit omvat zowel technische beperkingen, zoals datakwaliteit of fragmentatie, als organisatorische uitdagingen zoals het ontbreken van expertise of geschikte tools.

Binnen het oplossingsdomein richt het onderzoek zich op de vraag welke analysetechnieken en tools haalbaar zijn voor voetbalclubs die niet beschikken over diepgaande data-ervaring.
Daarbij wordt bekeken welke methoden voldoende eenvoudig, robuust en praktisch bruikbaar zijn om in een realistische voetbalcontext toegepast te worden.
Tot slot wordt onderzocht welke factoren bepalend zijn voor een succesvolle en duurzame implementatie van datagedreven besluitvorming binnen voetbalclubs.
Hierbij gaat het zowel om organisatorische voorwaarden, zoals rollen en processen, als om technische aspecten, waaronder databeheer en toolselectie.
...
Het eindresultaat moet concreet toepasbaar zijn binnen een brede schaal van voetbalomgevingen en voldoende technische diepgang bevatten om als AI \& Data Engineering-project te gelden.
...
De meerwaarde ontstaat pas wanneer deze data op een gestructureerde en doordachte manier worden geanalyseerd en geïnterpreteerd~\autocite{Alamar2013}.
...
Hoewel deze benadering niet exclusief op voetbal gericht is, biedt ze een relevant theoretisch kader voor het begrijpen van datagedreven processen binnen voetbalclubs.
...
Binnen voetbalonderzoek ligt de focus vaak op prestatieanalyse en matchanalyse.
...
Verschillende studies richten zich op het gebruik van prestatie-indicatoren en key performance indicators (KPI's) binnen voetbal.
...
Daarnaast wordt vastgesteld dat er een kloof bestaat tussen academisch ontwikkelde analysetechnieken en de praktische toepassing ervan door coaches en technische staf.

Recente literatuur onderzoekt ook de toepassing van machine learning en geavanceerde analysetechnieken binnen voetbaldata-analyse.
Pappalardo et al.\ introduceren bijvoorbeeld een framework voor spelersevaluatie op basis van grootschalige wedstrijddata, waarbij multidimensionale prestatieprofielen worden opgesteld~\autocite{Pappalardo2019}.
Dergelijke benaderingen tonen het potentieel van data-gedreven modellen om objectieve inzichten te genereren, maar vereisen vaak een hoge mate van technische expertise en data-infrastructuur, wat niet voor alle voetbalclubs haalbaar is.
...
Samenvattend toont de literatuur aan dat voetbalclubs beschikken over een grote hoeveelheid potentieel waardevolle data, maar dat de effectieve benutting ervan sterk afhankelijk is van toegankelijke analysetechnieken, organisatorische randvoorwaarden en praktische toepasbaarheid.
Deze vaststellingen onderstrepen de nood aan onderzoek dat zich richt op het ontwikkelen van haalbare, datagedreven oplossingen die aansluiten bij de realiteit van voetbalclubs zonder gespecialiseerde data-afdelingen.
...
Elke fase is afgestemd op het beantwoorden van de onderzoeksvragen en draagt op zijn beurt bij aan de ontwikkeling van een technisch onderbouwde en praktisch haalbare oplossing voor datagedreven besluitvorming binnen voetbalclubs.
...
% \end{itemize}
In de eerste fase wordt de huidige situatie omtrent datagebruik binnen voetbalclubs onderzocht.
...
Tijdens deze interviews wordt onderzocht welke soorten data momenteel worden verzameld (zoals wedstrijdstatistieken, GPS- en trackingdata, trainingsbelasting en blessuregegevens), hoe deze data worden opgeslagen en in welke mate ze effectief worden gebruikt ter ondersteuning van sportieve en operationele beslissingen.
Door deze combinatie van literatuur en praktijkobservaties ontstaat een helder beeld van de uitgangspositie en van de mate waarin data vandaag structureel worden ingezet.
...
Hierbij wordt rekening gehouden met realistische randvoorwaarden zoals beperkte middelen, tijdsdruk en het kennisniveau van de betrokken gebruikers.
...
% \end{itemize}
De derde fase bestaat uit de ontwikkeling van een proof-of-concept dat aantoont hoe voetbaldata op een toegankelijke en reproduceerbare manier geanalyseerd kunnen worden.
Hiervoor wordt een relevante dataset geselecteerd, afkomstig uit een voetbalclub of uit open datasets met wedstrijd- of eventdata.
De data worden eerst voorbewerkt en gestructureerd, waarna verschillende analysetechnieken worden toegepast.
De analyses focussen op haalbare en interpreteerbare methoden, zoals beschrijvende statistiek, regressiemodellen, clusteringtechnieken en eenvoudige anomaliedetectie voor het signaleren van afwijkende prestatie- of beslissingspatronen.
...
De totale verwachte doorlooptijd bedraagt ongeveer veertien weken. Deze doorlooptijd is opgesplitst in onderzoek, ontwikkeling, analyse en rapportering.
...
Het onderzoek wordt verwacht een helder en diepgaand inzicht op te leveren in de manier waarop voetbalclubs hun bestaande data efficiënter en effectiever kunnen benutten, zowel ter ondersteuning voor sportieve doeleinden als van operationele en strategische besluitvorming.
Tijdens de literatuurstudie en de analyse van de huidige praktijken wordt duidelijk welke soorten data vandaag de grootste impact hebben op prestatie-evaluatie, trainingsopvolging en wedstrijdanalyse.
...
Daarnaast zal het onderzoek een gedetailleerd beeld schetsen van de voornaamste knelpunten waarmee voetbalclubs geconfronteerd worden bij het verwerken, analyseren en interpreteren van hun data.
Deze knelpunten situeren zich zowel op technisch vlak, bijvoorbeeld inzake datakwaliteit, data-integratie en beschikbaarheid, als op organisatorisch vlak, waaronder beperkte analytische expertise, tijdsdruk en het ontbreken van geschikte tools.
Door deze obstakels systematisch in kaart te brengen ontstaat een duidelijk inzicht in de randvoorwaarden die nodig zijn om datagedreven werken binnen voetbalclubs succesvol te implementeren.
...
Op die manier draagt deze bachelorproef bij aan een professionelere en meer datagedreven benadering van prestatieanalyse en besluitvorming binnen het voetbal.
